...
export function createFromRequest({clientComponents, serverComponents, reqId, hash}: CreateFromRequestProps): Fingerprint {
    return {
        _id: nanoid(),
        reqId,
        hash,
...
export type Fingerprint = ServerFingerprintComponent & ClientFingerprintComponent & {
    _id: string;
    reqId: string;
    hash: string;
}
