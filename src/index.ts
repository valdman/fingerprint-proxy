...
app.get("*", function next(req, res) {
  ...
  const DAY = 24 * HOUR;
  const updatedExpiresDate = new Date(
    Date.now() + DAY * COOKIE_LIVETIME_IN_DAYS
  );
  ...
  const serverFingerprintComponents = (req.fingerprint
    .components as unknown) as ServerFingerprintComponent;
...
app.post("/fingerprint", async function next(req, res) {
  ...
  const serverComponents = (req.fingerprint
    .components as unknown) as ServerFingerprintComponent;
